...
if [[ "$1" != "2022" && "$1" != "2023" ]]; then
 echp "Blad. Pierwszy parametr musi byc rokiem (2022 lub 2023)"
 exit 1
fi

if ! [[ "$2" =~ ^[0-9]+$ ]] || ! [[ "$3" =~ ^[0-9]+$ ]]; then
 echo "Blad. Drugi i trzeci paramentr musza byc numeryczne."
 exit 1
fi
...
plik_lista_zakupow="Powtorzenie/Przepisy/lista_zakupow_$1.txt"
